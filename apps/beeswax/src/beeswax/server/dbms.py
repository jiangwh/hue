...
class HiveServer2Dbms(object):
  ...
  def get_sample(self, database, table):
    ...
    if not table.is_view:
      ...
      if table.partition_keys:
        partitions = self.get_partitions(database, table, partition_spec='', max_parts=1)
  ...
  def get_partitions(self, db_name, table, partition_spec, max_parts=None, reverse_sort=True):
  ...
  def get_partition(self, db_name, table_name, partition_id):
    ...
    partitions = self.get_partitions(db_name, table, partition_spec='', max_parts=None)
